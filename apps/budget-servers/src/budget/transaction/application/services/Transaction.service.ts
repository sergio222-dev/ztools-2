...
export class TransactionService {
  ...
  async findAllBy(
    ...
    arrFilters.push(byUser);

    if (accountIds) {
      const byAccount = new Filter('accountId', FilterOperator.fromValue(Operator.IN), accountIds);
      arrFilters.push(byAccount);
    }

    const filters = new Filters(arrFilters);
    const criteria = new Criteria(filters, Order.fromValues(), limit, (page - 1) * limit);
    return this.transactionRepository.matching(criteria);
  }
